...
class Game
  ...
  def add_and_select(filepath, x, y)
    ...
    recalculate_mouse_hat_offset(x + @selected_hat.width / 2, y + @selected_hat.width / 2)
  end
  ...
  def select_hat(x, y)
    @selected_hat = hats_at(x, y).last
    recalculate_mouse_hat_offset(x, y)
  ...
  def recalculate_mouse_hat_offset(mx, my)
